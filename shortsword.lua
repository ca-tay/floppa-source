...
local hat = chr["MeshPartAccessory"].Handle
hat:BreakJoints()
...
Weld.C0 = CFrame.new(0,-1.5,-2.4)*CFrame.Angles(math.rad(6),math.rad(90),90) -- Angles
hat.CanCollide = true
...
--                h:TakeDamage(Damage)
                ...
                ShowDamage((Part.CFrame * CFrame.new(0, 0, (Part.Size.Z / 0)).p + Vector3.new(0, 1.5, 0)), -Damage, 1.5, Part.BrickColor.Color)
                else
...
local con=Hitbox.Touched:connect(function(hit) Damagefunc(Hitbox,hit,0,0,math.random(0,0),"Normal",RootPart,0,0) end) 
...
MagniDamage(wave,0,0,0,math.random(0,0),"Knockdown",RootPart)
for i=0,80 do
swait()
...
MagniDamage(efprt,0,0,0,math.random(0,0),"Knockdown",RootPart)
...
delay(0.5,function ()
MagniDamage(Torso,0,0,0,math.random(0,0),"Knockdown",RootPart)
end)
...
function SolSiderea()
	...
	MagniDamage(ref,0,0,0,0,"Knockdown",ref)
	if hit~=nil then
	Damagefunc(ref,hit,0,0,0,"Knockdown",RootPart,0)
	end
	local n=2
